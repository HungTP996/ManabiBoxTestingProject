...
def quiz_data():
    """ファイル 'suimi_data.json' からすべてのデータをロードする。"""
    current_file_path = Path(__file__)
    suimi_dir = current_file_path.parent
    json_path = suimi_dir / "suimi_data.json"
...
def drawing_page(logged_in_page: Page):
    """適切な描画ページへ遷移するためのフィクスチャ。"""
    page = logged_in_page
    with allure.step("国語プレテスト (Suimi) の描画ページへ移動"):
        page.get_by_alt_text("国語").click()
        page.get_by_text("プレテスト").nth(9).click()
    yield page
