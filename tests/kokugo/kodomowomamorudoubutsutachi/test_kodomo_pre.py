...
def drawing_page(logged_in_page: Page):
    page = logged_in_page
    with allure.step("Kokugo Pre-test (kodomowo) 描画ページに移動"):
...
def test_full_quiz_flow(drawing_page: Page, quiz_data: dict):
    ...
    with allure.step(f"パート1: {total_drawing_questions} 問の描画問題を実行"):
        for i, question_data in enumerate(drawing_data_list):
            ...
            with allure.step(f"描画問題 {i + 1}/{total_drawing_questions}: 「{word}」"):
                with allure.step("Canvas が準備完了になるのを待機 (Visibility + Timeout)"):
                    canvas_locator = page.locator("canvas").first
                    expect(canvas_locator).to_be_visible(timeout=15000)
                    page.wait_for_timeout(2000)

                draw_character(page, question_data)
                confirm_score_and_proceed(page)

        print(f"{total_drawing_questions} 問の描画問題を完了しました。パート2に進みます...")
    ...
    with allure.step(f"パート2: {total_choice_questions} 問の選択問題を実行"):
        for index, question in enumerate(choice_questions):
            with allure.step(f"選択問題 {question['id']} (タイプ: {question.get('type')})"):
