...
def get_test_cases(path, topic_name):
    with open(path, 'r', encoding='utf-8') as f:
        all_data = json.load(f)
    return [case for case in all_data if case.get("topic") == topic_name]

test_data_path = 'tests/keisan/ichigakunen/data.json'

# kazutosuujiテストの本体
class TestGo:
    def verify_happycase_kazutosuuji(self, go_quiz_page: Page):
        page = go_quiz_page
        test_cases = get_test_cases(test_data_path, "kazutosuuji")
